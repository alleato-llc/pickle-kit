...
- **Scenario Outline grouping in reports** — `OutlineExpander` tags each expanded `Scenario` with `outlineName`/`exampleLabel`, threaded into `ScenarioResult`. The report/spec fold consecutive same-outline cases under one collapsible header (name + `outline · N`), via `ReportShared.segments(_:)`. Collapsed by default; a group with a failure opens; per-scenario anchors are preserved. `.github/workflows/pages.yml` generates the report+spec on every push from the cucumber feature fixtures — run by `GherkinIntegrationTests` in one deterministic pass (`swift test --filter GherkinIntegrationTests` with `PICKLE_REPORT`), NOT as a side effect of the whole 253-test suite (which would capture an arbitrary auto-collected slice). Those scenarios all pass, so the step is honestly gated (no `|| true`). It then emits a standalone theme-pinned copy of the report per palette (`report-{dark,nord,gruvbox}.html` — the pre-paint's `forced` value hard-set so the page ignores query/localStorage/OS), screenshots the spec + all four report pages (`report{,-dark,-nord,-gruvbox}.png`), and deploys to GitHub Pages ([alleato-llc.github.io/pickle-kit](https://alleato-llc.github.io/pickle-kit/)); the README's report screenshots are produced there, never committed.
- **Report theming** — `ReportShared.themes` is a list of `ReportTheme` (id, label, CSS-variable body); `paletteCSS()` emits one `:root[data-theme="…"]` block each. Ships `light`/`dark` (the ◐-toggle + OS-preference pair) plus `nord`/`gruvbox` (re-skinning showcase). The pre-paint script resolves `?theme=<id>` → `localStorage` → `prefers-color-scheme`, so a hosted page can be pinned to a palette. Add a theme = append one entry; every component reads the tokens.
